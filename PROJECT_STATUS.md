...
> **최종 업데이트:** 2026-03-12 (7차 업데이트 — 운영 신뢰성 강화 완료)
...
| 카카오 콜백 | KakaoCallbackPage.tsx | ✅ 완료 (dead code 정리됨) | Worker 서버사이드 처리 후 `/?token=` 리다이렉트 | 이 SPA 페이지는 정상 OAuth 흐름에서 도달 불가 — 에러/직접접근 시 `/login` 이동 |
...
| 책 등록 플로우 | RegisterFlowPage.tsx | ✅ 완료 | **useBookSearch + useAddBook** | 검색→선택→상태→저장 4단계 플로우, 완료 후 `/` 이동 |
