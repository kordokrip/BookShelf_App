...
> **분석 기준**: TRACE_MAP.md + PROJECT_STATUS.md 전수 분석 (2026-03-28)
...
**오프라인 상태 UI 부재**
PWA Workbox에 `NetworkFirst`(API, 5분 캐시)와 `CacheFirst`(이미지, 30일) 설정은 있으나, 네트워크 단절 시 사용자에게 오프라인 상태를 알려주는 UI 피드백이 없다. `navigator.onLine` 이벤트 기반 Toast/Banner가 누락되어 있다.
...
#### RISK-F01: BottomNavBar 이중 쿼리 — 렌더링 비용
...
#### RISK-F04: 오프라인 상태 처리 부재

Workbox `NetworkFirst` 캐시가 있어 API 응답은 5분간 캐싱되나, 캐시 만료 후 네트워크 단절 상태에서는 `apiFetch`가 `TypeError: Failed to fetch`를 throw한다. `queryClient`의 retry 로직은 이를 5xx로 처리하여 2회 재시도하지만 사용자에게 아무런 UI 피드백이 없다.

---

### 3-2. 백엔드 및 API 리스크

#### RISK-B01: 비밀번호 해싱 강도 부족
...
#### RISK-D01: notes 전문 검색 — 데이터 증가 시 Full Scan
...
#### RISK-D02: StatsPage — books 전체 로드 후 클라이언트 집계
...
#### QW-01: 독서 스트릭 계산 함수 추가 (`StatsPage.tsx`)
...
#### QW-02: 오프라인 상태 감지 Toast (`App.tsx`)
...
#### QW-03: Rate Limiting 미들웨어 (Worker KV 기반)
...
#### QW-04: `DesignSystemPage` 프로덕션 접근 차단
...
#### NL-03: `GET /api/stats` 전용 엔드포인트 도입
...
|------|----------|----------|---------|
| 비밀번호 해싱 | SHA-256 | PBKDF2 (600k iterations) | 0.5일 |
| 노트 검색 | LIKE '%keyword%' | FTS5 가상 테이블 | 1일 |
| Stats API | 4개 쿼리 분산 | 1개 집계 엔드포인트 | 0.5일 |
| Rate Limiting | 없음 | KV 기반 슬라이딩 윈도우 | 0.5일 |
| 오프라인 UI | 없음 | online/offline 이벤트 Toast | 0.25일 |
| 독서 타이머 | 없음 | useReadingTimer 훅 | 0.5일 |
| 독서 스트릭 | 없음 | 클라이언트 집계 함수 | 0.25일 |
| Push Notification | 없음 | VAPID + KV 구독 저장 | 2일 |
| Refresh Token | 없음 (SESSIONS KV 선언만) | 30일 Refresh | 1일 |
| Background Sync | 없음 | Workbox BackgroundSync | 1일 |
| Google OAuth | 버튼 disabled | Worker 엔드포인트 구현 | 1일 |
...
핵심 개선 우선순위는 다음 순서로 접근하면 효과적이다:

1. **즉시** — Rate Limiting(보안), 오프라인 Toast(UX), `staleTime` 튜닝(성능)
2. **1주 이내** — 독서 타이머·스트릭(차별점 강화), PBKDF2 마이그레이션(보안)
3. **2~4주** — FTS5 노트 검색, Stats 전용 API, Web Push, Background Sync
