import { Crown, Trash2, LogOut, UserMinus, ArrowRightLeft } from 'lucide-react';
import { useLeaveGroup, useDeleteGroup, useRemoveMember, useTransferLeader } from '../../../hooks/useGroups';
import { useAuthStore } from '../../../stores/authStore';

export function MembersTab({ groupId, members, isLeader, ownerId, onBack }: {
  groupId: string;
  members: Array<{ user_id: string; user_name: string; role: string; profile_emoji?: string; joined_at: string }>;
          ...
          <div key={m.user_id} className="flex items-center gap-3 p-3 bg-white dark:bg-[#1E293B] rounded-xl border border-[#E2E8F0] dark:border-[#334155]">
            ...
            {isLeader && m.user_id !== user?.id && (
              <div className="flex items-center gap-1">
                <button
                  onClick={() => handleTransferLeader(m.user_id, m.user_name)}
